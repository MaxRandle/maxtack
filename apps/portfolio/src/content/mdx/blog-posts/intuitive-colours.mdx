...
This does NOT represent luminosity. Remember the CIELAB colour space has no relation to the physical properties of light. It represents how "light" a colour is appears. This is expressed as a percentage from 0% and 100%, with 0% being black and 100% being white.
...
By setting lightness to 70%, youre saying: "I want a colour that humans will perceive as being 70% light". The colour will always be percieved as such regardless of the hue or chroma. The same is true for chroma and hue.

A product of this is that each variable is completely independent of the others. This makes LCH extremely easy and intuitive to work with because we can tweak each variable in isolation.
...
1. Generate an OKLCH colour for each shade and hue combination using a sass loop.
...
```scss
--emerald-600: oklch(74% 0.23 155);
--sky-1100: oklch(40% 0.16 230);
```
